...
/* Filter out planets based on the following criteria for potential habitability:
...
const habitablePlanets = planets.filter(function (value) {
  return (
    value.temperature >= 253 &&
    value.temperature <= 323 &&
    value.distance >= 0.75 &&
    value.distance <= 1.5
  );
});

const habitablePlanetsNames = habitablePlanets.map(function (planet) {
  return planet.name;
});
