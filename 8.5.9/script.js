document.addEventListener("DOMContentLoaded", function ()
{
	...
	const savedNoteIdCounter = Number(localStorage.getItem("noteIdCounter"));
	noteIdCounter = Number.isNaN(savedNoteIdCounter) ? 0 : savedNoteIdCounter;
	...
	function addNewNote ()
	{
		...
		note.setAttribute("data-note-id", String(id)); // Stores the note ID to its data attribute.
		...
		localStorage.setItem("noteIdCounter", String(noteIdCounter));
	...
	document.addEventListener("dblclick", function (event)
	{
		if (event.target.classList.contains("note"))
		{
			...
			localStorage.remove("savedNotes");
		}
	});
